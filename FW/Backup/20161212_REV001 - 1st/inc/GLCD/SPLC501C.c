...
void GLCD_WriteChar19x24(char charCode) {
    ...
    for (fontCollumn = 1; fontCollumn < FONT_WIDTH19x24; fontCollumn+=3) {
        ...
        else GLCD_WriteData(font19x24[((charCode - FONT_OFFSET19x24) * FONT_WIDTH19x24) + fontCollumn]);
        GLCD_GoTo(x,y+1);
		if (charCode == ' ') GLCD_WriteData(0x00);
        else GLCD_WriteData(font19x24[((charCode - FONT_OFFSET19x24) * FONT_WIDTH19x24) + fontCollumn + 1]);
        GLCD_GoTo(x,y+2);
		if (charCode == ' ') GLCD_WriteData(0x00);
        else GLCD_WriteData(font19x24[((charCode - FONT_OFFSET19x24) * FONT_WIDTH19x24) + fontCollumn + 2]);
        x++;
    }
    GLCD_WriteData(0);
	if (charCode == '.') x -= 5;
